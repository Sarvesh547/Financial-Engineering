...
def mainCalculation():
    ...
    #compare ZCB from mkt and obtained 
    Exact = np.zeros([N,1])
    Proxy= np.zeros ([N,1])
    ...
    # calculation of money saving account
    ...
    P_T1_T2 = HW_ZCB(lambd,eta,P0T,T1,T2,r[:,-1]) #hull-white simulation
    ...
    frwd = 1.0/(T2-T1) *(P0T(T1)/P0T(T2)-1.0) #forward rate
    ...
    plt.legend(legend)        
     ...
     # Effect of lambda(mean reversion)
    plt.figure(4)
    plt.grid()
    plt.xlabel('strike, K')
    plt.ylabel('implied volatility')
